...
async def chat_with_shortlist(job_id: str, user_question: str) -> str:
    ...
    if not candidates:
        ...
        # FIX: Limit to Top 5 and max 1000 chars to prevent context overflow on local LLMs
        top_candidates = candidates[:5] 
        if len(candidates) > 5:
            print(f"Warning: Truncating shortlist from {len(candidates)} to Top 5 for chat context.")
        ...
        for c in top_candidates:
            ...
            clean_text = c['text'].replace('\n\n', '\n')[:1000] # Cap at 1k chars (~300 tokens)
